...
class Ava(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, split):
        ...
        # Label array only have 8 classes Here
        self.cfg = cfg
        self._split = split
        ...
        self._filter_labels = [1, 3, 10, 15, 54, 57, 68, 69]
        self._map_labels = {1: 1, 3: 2, 10: 3, 15: 4, 54: 5, 57: 6, 68: 7, 69: 8}
        # Augmentation params.
    ...
    def filter_boxes_and_labels(self, boxes_and_labels):
        result = []
        for video_idx in range(len(self._image_paths)):
            new_frame_sec = {}
            for frame_sec, boxes in boxes_and_labels[video_idx].items():
                # boxes = [ [box=[x1,x2,y1,y2], labels=[label1, label2, ...]], ... ]
                new_boxes = []
                for box, labels in boxes:
                    new_labels = []
                    for label in labels:
                        if label in self._filter_labels: new_labels.append(self._map_labels[label])

                    if len(new_labels) > 0: new_boxes.append([box, new_labels])
                if len(new_boxes) > 0: new_frame_sec[frame_sec] = new_boxes
            if new_frame_sec: result.append(new_frame_sec)
    ...
    def _load_data(self, cfg):
        ...
        """
        # Loading frame paths.
        # _image_paths = ['/mnt/...../video_name/-5KQ66BBWC4/-5KQ66BBWC4_0000000001.jpg', ...]
        # _video_idx_to_name = ['-5KQ66BBWC4', ....]
        (
            self._image_paths,
        ...
        # {'video_name': {'frame_sec': [ [ ['x1','x2','y1','y2'], [label] ], ... ] } }
        boxes_and_labels = [
            boxes_and_labels[self._video_idx_to_name[i]]
            for i in range(len(self._image_paths))
        ]

        if self._split == "train": boxes_and_labels = self.filter_boxes_and_labels(boxes_and_labels)
        # Get indices of keyframes and corresponding boxes and labels.
        # _keyframe_indices = [( video_idx, sec_idx, sec, sec_to_frame(sec) ), ...]
        # _keyframe_boxes_and_labels = [boxes_and_labels[video_idx][sec]]
        (
    ...
    def filter_clip_labels(self, clip_label_list):
        result = []
        for box_labels in clip_label_list:
            new_labels = []
            for label in box_labels[1]:
                if label in filter_labels: new_labels.append(map_labels[label])

            if len(new_labels) > 0: result.append([box_labels[0], new_labels])
    ...
    def __getitem__(self, idx):
        ...
        # clip_label_list = self.filter_clip_labels(clip_label_list)
