...
def load_checkpoint(
    ...
    ms = model if not data_parallel else model.module
...
def load_train_checkpoint(cfg, model, optimizer, data_parallel=True):
    """
    Loading checkpoint logic for training.
    """
    if not cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        last_checkpoint = get_last_checkpoint(cfg.OUTPUT_DIR)
        logger.info("Load from last checkpoint, {}.".format(last_checkpoint))
        checkpoint_epoch = load_checkpoint(
            last_checkpoint, model, data_parallel, optimizer
        )
